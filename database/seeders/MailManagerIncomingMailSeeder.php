...
class MailManagerIncomingMailSeeder extends Seeder
{
    private const EXPECTED_RECORDS = 19897;
    ...
    public function run(): void
    {
        ...
        [$inserted, $skipped] = DB::transaction(function () use ($capturedBy, $records): array {
            ...
            foreach ($records as $record) {
                ...
                $pending[] = [
                    'direction' => 'incoming',
                    'register_number' => sprintf('IM-MMP-%05d', $record['sequence']),
        ...
        $this->command?->info(
            "Mail Manager incoming mail: {$inserted} inserted, {$skipped} already present."
        );
    }
    ...
    {
        $path = database_path('seeders/data/mail_manager_incoming_mail_seed.json.gz');
        $compressed = file_get_contents($path);
        $contents = $compressed === false ? false : gzdecode($compressed);

        if ($contents === false) {
            throw new RuntimeException("Unable to read Mail Manager incoming-mail seed data at {$path}.");
        }
        ...
        if (
            ...
            || ($records[array_key_last($records)]['source_page'] ?? null) !== 985
        ) {
            throw new RuntimeException(
                'The Mail Manager incoming-mail seed must contain 19,897 ordered records across 985 pages.'
            );
        }
    ...
    {
        return sprintf('mail-manager-pdf-2026-07-22:%05d', $sequence);
    }
    ...
    {
        $rawDate = $record['received_date_raw'];
        $location = "Mail Manager PDF page {$record['source_page']}, row {$record['source_row']}.";

        if ($rawDate === '') {
            return "Source: {$location} Printed received date was blank.";
        }

        if (preg_match('/\s0\d{3}$/', $rawDate) === 1) {
            return "Source: {$location} Printed received date '{$rawDate}' was normalized to {$record['received_date']}.";
        }
